...
11. Какие ключи могут быть в мапе?

				Все сравнимые типы данных. Числа, строки, булевые значения, массивы, каналы, структуры (если все его поля сравнимы)
...
sync.Map - прокачанная мапа. Предназначена для потокобезопасного работы с горутинами. Предоставляет готовый набор методов для конкурентного пользования.

Оптимизированная для сценариев с большим количеством операций чтения и стабильными ключами. Она использует две внутренние карты: read (только для чтения) и dirty (для записи и чтения), что позволяет минимизировать блокировки при чтении.

**Реализация**
Структура — sync.Map использует две внутренние карты: read и dirty. Read используется для быстрого доступа к данным без блокировок, а dirty — для записи и чтения с блокировкой.
Операции — При чтении sync.Map сначала проверяет read карту. Если ключ не найден, проверяется dirty карта с захватом мьютекса.
Обновление — При записи sync.Map сначала пытается обновить read карту. Если это невозможно, обновляется dirty карта с захватом мьютекса.
Продвижение — Когда количество промахов при чтении из read карты превышает длину dirty карты, содержимое dirty копируется в read, что ускоряет последующие операции чтения.
Применение — sync.Map подходит для сценариев с частыми чтениями и стабильными ключами, например, для **кэшей**.

**Почему sync.Map, а не map + Mutex?**
**Очень** много горутин одновременно читают и записывают в карту, sync.Map может быть быстрее, чем обычная map + sync.Mutex, за счёт своей внутренней оптимизированной структуры и механики. Плюсом, она хорошо работает со сценарием, когда требуется регулярный полный обход при не очень частых записях.
Готовая потокобезопасность «из коробки»: не нужно явно ставить мьютексы.

**Почему map + Mutex, а не sync.Map?**
При большом количестве записей/удалений (структурных изменений) на помощь приходит map + Mutex.
Мы создаём обычную Go-карту и оборачиваем её мьютексом. Позволяет чётко контролировать, где именно мы ставим блокировку (можно использовать sync.RWMutex для разделения чтения и записи, что ещё улучшит производительность при большом числе читателей).
Незначительное количество горутин.
